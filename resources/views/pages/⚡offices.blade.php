...
<section class="flex h-full w-full flex-1 flex-col gap-6">
    ...
    <form wire:submit="save" class="rounded-lg border border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900">
        <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            ...
            <flux:select wire:model.live="countryId" :label="__('País')" data-test="office-country">
                <flux:select.option value="">{{ __('Selecciona país') }}</flux:select.option>
                @foreach ($this->countries as $country)
                    <flux:select.option :value="$country->id" :key="$country->id">{{ $this->countryLabel($country) }}</flux:select.option>
                @endforeach
            </flux:select>
            ...
            >
                <flux:select.option value="">{{ $countryId ? __('Selecciona ciudad') : __('Selecciona país primero') }}</flux:select.option>
                @foreach ($this->cities as $city)
                    <flux:select.option :value="$city->id" :key="$city->id">{{ $city->name }} · {{ $city->code }}</flux:select.option>
                @endforeach
            </flux:select>
            ...
            >
                <flux:select.option value="">{{ $cityId ? __('Selecciona zona') : __('Selecciona ciudad primero') }}</flux:select.option>
                @foreach ($this->zones as $zone)
                    <flux:select.option :value="$zone->id" :key="$zone->id">{{ $zone->name }} · {{ $zone->code ?? '-' }}</flux:select.option>
                @endforeach
            </flux:select>
    ...
    <div class="flex flex-col gap-3 rounded-lg border border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900">
        <div class="flex flex-col gap-3">
            ...
            <div class="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                <flux:select wire:model.live="filterCountryId" :label="__('País')" data-test="office-filter-country">
                    <flux:select.option value="">{{ __('Todos') }}</flux:select.option>
                    @foreach ($this->filterCountries as $country)
                        <flux:select.option :value="$country->id" :key="$country->id">{{ $this->countryLabel($country) }}</flux:select.option>
                    @endforeach
                </flux:select>
                ...
                <flux:select
                    ...
                    wire:model.live="filterCityId"
                    :label="__('Ciudad')"
                    :disabled="! $filterCountryId"
                    data-test="office-filter-city"
                >
                    <flux:select.option value="">{{ $filterCountryId ? __('Todas') : __('Selecciona país primero') }}</flux:select.option>
                    @foreach ($this->filterCities as $city)
                        <flux:select.option :value="$city->id" :key="$city->id">{{ $city->name }} · {{ $city->code }}</flux:select.option>
                    @endforeach
                </flux:select>
                ...
                <flux:select
                    ...
                    wire:model.live="filterZoneId"
                    :label="__('Zona')"
                    :disabled="! $filterCityId"
                    data-test="office-filter-zone"
                >
                    <flux:select.option value="">{{ $filterCityId ? __('Todas') : __('Selecciona ciudad primero') }}</flux:select.option>
